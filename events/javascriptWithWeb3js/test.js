...
async function getAbi(signature, silent) {
  ...
  if (jsonResponse.count == 1) {
    ...
    if (!silent) {
      console.error(
        "response from www.4byte.directory resulted in too many results"
      );
    }
  }
  return "";
    ...
    if (errorFunction != "") {
      ...
      if (decodedError) {
        ...
        decodedError.params.forEach((parameter) => {
          console.log(
            `${".".repeat(indent)}Parameter (${parameter.type}) = ${
              parameter.value
            }`
          );
          ...
          if (parameter.type == "address") {
            console.log(
              `${".".repeat(
                indent
              )}=> Hedera address ${AccountId.fromSolidityAddress(
                parameter.value
              )}`
            );
          }
...
async function main() {
  ...
  if (args.length == 2) {
    ...
    console.error(
      "invalid command line arguments supplied, please consult README.md"
    );
  }
}
