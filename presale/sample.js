...
async function main() {
  ...
  const contractBytecode = readFileSync("hbarToAndFromContract.bin");
...
====================================================`);
  ...
  const cRx = await contractExecuteFcn(
    contractId,
    gasLimit,
    "callHbar",
    cParams,
...
async function showContractBalanceFcn(cId) {
  ...
  console.log(
    `- Contract balance (ContractInfoQuery SDK): ${info.balance.toString()}`
  );
}
